    ...
    def _sounddevice_callback(
        ...
        if self._loop and self._loop.is_running():
            asyncio.run_coroutine_threadsafe(
                self._queue.put(self._create_frame(indata)), self._loop
            )
    ...
    def _create_frame(self, indata: np.ndarray) -> Frame:
        ...
        if audio_data.dtype != self.dtype:
            audio_data = (
                ...
                else audio_data.astype(self.dtype)
            )
        frame = AudioFrame(
            format="s16",
            layout="mono",
        ...
        try:
            while True:
                ...
                await self.realtime_client.send(
                    RealtimeAudioEvent(
                        audio=AudioContent(
                            data=base64.b64encode(cast(Any, data)).decode("utf-8")
                        )
                    )
                )
...
class AudioPlayerWebRTC(BaseModel):
    ...
    async def add_audio(self, audio_content: AudioContent) -> None:
        ...
        if not self._queue:
            ...
            return
        if audio_content.inner_content and isinstance(
            audio_content.inner_content, AudioFrame
        ):
        ...
        if isinstance(audio_content.data, str):
            await self._queue.put(
                np.frombuffer(audio_content.data.encode(), dtype=self.dtype)
            )
            return
